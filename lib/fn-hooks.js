		...
		function next(hook, index, args) {
			...
			if (fnWrapped[methodName][hook][index] === undefined) {
				...
				args.unshift(function () {
					//Merge with default arguments (the +1 is so we splice out the next() callback during the merge)
					args.splice(0, arguments.length + 1, _.toArray(arguments));
