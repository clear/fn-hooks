...
module.exports = function (object) {
	...
	//Fake calling a method to trigger all hooks
	object.trigger = function () {
		//First argument will be method name
		var methodName = Array.prototype.splice.call(arguments, 0, 1);

		//Only works if we've defined hooks
		if (fnWrapped[methodName]) {
			//Temporarily wrap the inner function
			fnWrapped[methodName].fn = _.wrap(fnWrapped[methodName].fn, function () {
				//Don't do anything, simply restore by re-applying fn
				fnWrapped[methodName].fn = Array.prototype.splice.call(arguments, 0, 1);
			...
			//Now actually fire function
			object[methodName].apply(this, arguments);
		}
	};
